...
// [showScreen replaced by unified version below]
...
// ════════════════════════════════════════════════
// 🆕 ДОБАВЕНИ ФУНКЦИИ — редизайн 2026-03
// ════════════════════════════════════════════════

// ── showScreen — единна функция за всички екрани ─────────────
window.showScreen = function(screen) {
  ...
  if (screen === "report") {
    ...
    loadTasksRealtime(); // стартира Firebase listener (само веднъж)
    renderTasks();
    checkNotifStatus();
...
// ── renderRecentList — card-based списък (мобилно) ───────────
...
function renderStoreComparison() {
  ...
  // "1"/"2" → магазин; null → Каса/празно (не изкривява М1/М2)
  const ns = (s) => {
    const v = String(s ?? "").trim().toLowerCase();
    if (v === "1" || v === "м1" || v.includes("магазин 1")) return "1";
    if (v === "2" || v === "м2" || v.includes("магазин 2")) return "2";
  ...
  const ks = kasaInc - kasaExp;
  const tI = m["1"].inc + m["2"].inc + kasaInc;
  const tE = m["1"].exp + m["2"].exp + kasaExp;
  const tS = s1 + s2 + ks;
  ...
  const bar = (val,tot,col) => {
    ...
    return `<div style="height:3px;background:var(--border);border-radius:2px;margin-top:3px">
  ...
  el.innerHTML = `
    ...
    <table>
      <thead><tr>
        <th></th>${th("🏪 М1")}${th("🏪 М2")}${hk ? th("🏧 Каса") : ""}${th("📊 Общо")}
      </tr></thead>
      <tbody>
        <tr>
          <td style="color:var(--text2)">Приходи</td>
          ${td2(m["1"].inc,"pos")}${td2(m["2"].inc,"pos")}${hk ? td2(kasaInc,"pos") : ""}
          <td style="text-align:right;font-family:var(--mono);font-weight:700;color:var(--green)">${f(tI)}</td>
        </tr>
        <tr>
          <td style="color:var(--text2)">Разходи</td>
          ${td2(m["1"].exp,"neg")}${td2(m["2"].exp,"neg")}${hk ? td2(kasaExp,"neg") : ""}
          <td style="text-align:right;font-family:var(--mono);font-weight:700;color:var(--red)">${f(tE)}</td>
        </tr>
...
// ── syncPills — синхронизира pill бутоните ────────────────────
...
// ── Бележки / задачи — Firebase sync ─────────────────────────
const tasksCol = collection(db, "tasks");
let _tasks = [];        // локален кеш, зареден от onSnapshot
let _tasksUnsub = null; // unsubscribe handle

// Стартира realtime listener — вика се при showScreen('notes')
function loadTasksRealtime() {
  if (_tasksUnsub) return; // вече слуша
...
window.addTask = async function() {
  ...
  try {
    ...
    // renderTasks ще се извика автоматично от onSnapshot
...
window.toggleTask = async function(firestoreId) {
  ...
  if (!task) return;
  try {
    await updateDoc(doc(db, "tasks", firestoreId), { done: !task.done });
...
function renderTasks() {
  ...
  if (!tasks.length) { el.innerHTML = '<div class="tasks-empty">Няма бележки — добави първата 👆</div>'; return; }
  ...
  el.innerHTML = [...tasks].sort((a,b) => Number(a.done)-Number(b.done)).map(t => `
...
window.toggleNotifications = async function(on) {
  ...
  if (on) {
    ...
    if (perm !== 'granted') { status.textContent = '⛔ Отказано'; document.getElementById('notifToggle').checked = false; localStorage.setItem('ns_notif','0'); return; }
...
function scheduleReminder() {
  ...
  setTimeout(() => {
    const today    = new Date().toISOString().slice(0, 10);
    const hasToday = typeof records !== 'undefined' && records.some(r => (r.date||'').startsWith(today));
