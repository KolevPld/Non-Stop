...
function applyFilters() {
  ...
  console.log("=== ФИЛТЪР ===", {type, method, category, startDate, endDate, store});
  console.log("Общо записи:", records.length);
  ...
  filteredRecords = records.filter(r => {
    ...
    const pass = matchType && matchMethod && matchCategory && matchStart && matchEnd && matchStore;
    if (!pass && store) {
      console.log("ИЗКЛЮЧЕН:", r.date, r.store, "→normStore:", normStore(r.store), "vs filter:", store,
        {matchType, matchMethod, matchCategory, matchStart, matchEnd, matchStore});
    }
    return pass;
  });

  console.log("След филтър:", filteredRecords.length, "записа");
