...
function updateFilterSummary(data) {
  ...
  const ns = (s) => {
    ...
    if (v === "1" || v === "м1" || v.includes("магазин 1")) return "М1";
    if (v === "2" || v === "м2" || v.includes("магазин 2")) return "М2";
    return "Каса";
  };
  ...
  data.forEach(r => {
    ...
    if (!stores[store]) stores[store] = { inc: 0, exp: 0 };
    const a = Number(r.amount || 0);
    if (r.type === "Приход") { stores[store].inc += a; totalInc += a; }
    else                     { stores[store].exp += a; totalExp += a; }
  });
  ...
  // Редове по магазин (само тези с данни)
  const storeRows = Object.entries(stores).map(([name, v]) => {
    const sal = v.inc - v.exp;
    return `
      <div class="fs-store-row">
  ...
  el.innerHTML = `
    <div class="filter-summary-box">
      <div class="fs-header">
        <span>📊 Резултат от филтъра — <strong>${data.length}</strong> записа</span>
      </div>
      <div class="fs-totals">
      ...
      ${Object.keys(stores).length > 1 ? `<div class="fs-stores">${storeRows}</div>` : ""}
    </div>`;
}
