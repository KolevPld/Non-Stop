...
function renderStoreComparison() {
  ...
  const nm = (m) => { const v = String(m ?? "").trim().split(" ")[0]; return v === "Карта" ? "Банка" : v; };
  const td = (iso) => {
    const p = String(iso ?? "").trim().match(/^(\d{4})-(\d{2})-(\d{2})$/);
    return p ? new Date(+p[1], +p[2]-1, +p[3]) : null;
  };

  // М1 и М2 — приходи/разходи + разбивка по метод
  const m = {
    "1": { inc:0, exp:0, кеш_inc:0, кеш_exp:0, банка_inc:0, банка_exp:0 },
    "2": { inc:0, exp:0, кеш_inc:0, кеш_exp:0, банка_inc:0, банка_exp:0 }
  };
  ...
  records.forEach(r => {
    ...
    if (!store) return; // Каса — пропускаме в сравнението
    const method = nm(r.method);
    const isInc = r.type === "Приход";
    if (isInc) m[store].inc += amount; else m[store].exp += amount;
    if (method === "Кеш") {
      if (isInc) m[store].кеш_inc += amount; else m[store].кеш_exp += amount;
    } else {
      if (isInc) m[store].банка_inc += amount; else m[store].банка_exp += amount;
    }
  });

  const s1 = m["1"].inc - m["1"].exp;
  const s2 = m["2"].inc - m["2"].exp;
  const tI = m["1"].inc + m["2"].inc;
  const tE = m["1"].exp + m["2"].exp;
  const tS = s1 + s2;

  // Общи суми по метод
  const totalКеш_inc   = m["1"].кеш_inc   + m["2"].кеш_inc;
  const totalКеш_exp   = m["1"].кеш_exp   + m["2"].кеш_exp;
  const totalБанка_inc = m["1"].банка_inc + m["2"].банка_inc;
  const totalБанка_exp = m["1"].банка_exp + m["2"].банка_exp;
  const totalКеш_sal   = totalКеш_inc   - totalКеш_exp;
  const totalБанка_sal = totalБанка_inc - totalБанка_exp;
  ...
  const sep = `<tr><td colspan="4" style="height:6px;border-top:1px solid var(--border);padding:0"></td></tr>`;

  el.innerHTML = `
    <h3><i class="fa-solid fa-scale-balanced"></i> Сравнение М1 vs М2 — текущ месец</h3>
    <table>
      <thead><tr><th></th>${th("🏪 М1")}${th("🏪 М2")}${th("📊 Общо")}</tr></thead>
      <tbody>
        <tr><td style="color:var(--text2)">Приходи</td>${td2(m["1"].inc,"pos")}${td2(m["2"].inc,"pos")}<td style="text-align:right;font-family:var(--mono);font-weight:700;color:var(--green)">${f(tI)}</td></tr>
        <tr><td style="color:var(--text2)">Разходи</td>${td2(m["1"].exp,"neg")}${td2(m["2"].exp,"neg")}<td style="text-align:right;font-family:var(--mono);font-weight:700;color:var(--red)">${f(tE)}</td></tr>
        ...
        <tr>
          ...
          <td style="text-align:right;font-family:var(--mono);font-size:.85rem" class="${cls(m["1"].кеш_inc - m["1"].кеш_exp)}">${f(m["1"].кеш_inc - m["1"].кеш_exp)}</td>
          <td style="text-align:right;font-family:var(--mono);font-size:.85rem" class="${cls(m["2"].кеш_inc - m["2"].кеш_exp)}">${f(m["2"].кеш_inc - m["2"].кеш_exp)}</td>
          <td style="text-align:right;font-family:var(--mono);font-weight:700;font-size:.9rem" class="${cls(totalКеш_sal)}">${f(totalКеш_sal)}</td>
        </tr>
        <tr>
          <td style="color:var(--text3);font-size:.8rem">🏦 Банка/Карта</td>
          <td style="text-align:right;font-family:var(--mono);font-size:.85rem" class="${cls(m["1"].банка_inc - m["1"].банка_exp)}">${f(m["1"].банка_inc - m["1"].банка_exp)}</td>
          <td style="text-align:right;font-family:var(--mono);font-size:.85rem" class="${cls(m["2"].банка_inc - m["2"].банка_exp)}">${f(m["2"].банка_inc - m["2"].банка_exp)}</td>
          <td style="text-align:right;font-family:var(--mono);font-weight:700;font-size:.9rem" class="${cls(totalБанка_sal)}">${f(totalБанка_sal)}</td>
        </tr>
      </tbody>
    </table>`;
}
