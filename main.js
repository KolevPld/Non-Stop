...
window.editImage = async function (id) {
  ...
  window.syncPills?.();
  const _ft = document.getElementById('formTitle');
  if (_ft) _ft.textContent = 'Редактирай запис';
  const _ip = document.getElementById('imagePreview');
  const _rb = document.getElementById('removeImgBtn');
  if (_rb) _rb.classList.toggle('hidden', !_ip?.src || _ip?.classList.contains('hidden'));
};
...
window.cancelEdit = function () {
  ...
  const _ft2 = document.getElementById("formTitle");
  if (_ft2) _ft2.textContent = "Нов запис";
  window.syncPills?.();
...
function renderMethodSummary() {
  ...
  const normalizeStore = (s) => {
    ...
    return null; // Каса, празно → не изкривява М1/М2
  };
  ...
  records.forEach((r) => {
    ...
    const store = normalizeStore(r.store);  // "1", "2" или null
