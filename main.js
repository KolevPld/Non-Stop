...
function renderMethodSummary() {
  const makeTotals = () => ({ Кеш: 0, Банка: 0 }); // Карта → Банка
  ...
  const normalizeMethod = (m) => { const v = String(m ?? "").trim().split(" ")[0]; return v === "Карта" ? "Банка" : v; }; // Карта → Банка
  const toDate = (iso) => {
    // очакваме "YYYY-MM-DD"
  ...
  if (ms) {
    ...
    const m1Bank = totalsByStoreMonth["1"].Банка;
    const m2Cash = totalsByStoreMonth["2"].Кеш;
    const m2Bank = totalsByStoreMonth["2"].Банка;

    const totalAll = m1Cash + m1Bank + m2Cash + m2Bank;

    ms.innerHTML = `
      <h3><i class="fa-solid fa-wallet"></i> Разпределение по метод (текущ месец)</h3>
      <table>
        ${row("💰 Кеш (М1):", `${fmt2(m1Cash)} €`)}
        ${row("🏦 Банка/Карта (М1):", `${fmt2(m1Bank)} €`)}
        ${row("💰 Кеш (М2):", `${fmt2(m2Cash)} €`)}
        ${row("🏦 Банка/Карта (М2):", `${fmt2(m2Bank)} €`)}
        ${row("Общо:", `${fmt2(totalAll)} €`, true)}
      </table>
    `;
  }
  ...
  if (msx) {
    ...
    const totalBank = totalsAllPeriod.Банка; // Карта вече → Банка
