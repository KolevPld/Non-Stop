...
- **Попълване на дневен отчет от снимка (планирано, 2026-09-23):**
  Callable Cloud Function `parseReportPhoto` → Gemini (`gemini-3.8-flash`) през Vertex AI,
  `location: "global"`, SDK `@google/genai`, БЕЗ API ключ (правата на service account-а).
  Снимки: разпечатка от касов софтуер + ръкописен лист + фактури от доставчици.
  Снимката НЕ се пази. Функцията НЕ пише във Firestore — само връща JSON.
  Клиент: бутон "📷 От снимка" → компресия до 2000px → модал за преглед с чекбокси →
  попълва формата (жълто `ocr-filled`), не презаписва тихо, разходите се добавят в празни редове.
  Записът е през съществуващия бутон; формулата на endCash НЕ се пипа.
  Предварително: включи Vertex AI API, роля roles/aiplatform.user на service account-а,
  бюджет с известие в Billing, `npm install @google/genai` във functions/.
