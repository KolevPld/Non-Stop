...
// 🔄 Филтри
// --------------------------------------------------
// filters се чете при всяко извикване
const filters = {};

// Нормализира store стойност към "1", "2" или "" 
function normalizeStoreVal(s) {
  const v = String(s ?? "").trim().toLowerCase();
  if (v === "1" || v === "м1" || v.includes("магазин 1") || v.includes("magazin 1")) return "1";
  if (v === "2" || v === "м2" || v.includes("магазин 2") || v.includes("magazin 2")) return "2";
  return v; // Каса или друго
}
...
function applyFilters() {
  ...
  filteredRecords = records.filter(r => {
    ...
    const matchMethod   = !method   || (String(r.method || "").split(" ")[0]) === method;
    ...
    const matchStore    = !store     || normalizeStoreVal(r.store) === store;
