...
// 🔄 Филтри
// --------------------------------------------------
// ── Помощни функции за нормализация ──────────────────────────
...
function normMethod(m) {
  // Взима само първата дума и нормализира Карта→Банка
  const v = String(m ?? "").trim().split(/\s+/)[0];
  return v === "Карта" ? "Банка" : v;
}

// ── Филтри — четат DOM при всяко извикване ────────────────────
const filters = {};
...
// Бутон "📅 Месец" — задава от/до за текущия месец
window.setCurrentMonth = function() {
  ...
  if (ed) ed.value = `${year}-${month}-${String(last).padStart(2,"0")}`;
  applyFilters();
};
...
function updateFilterSummary(data) {
  ...
  data.forEach(r => {
    const sKey = normStore(r.store) || "—";
    const label = sKey === "1" ? "М1" : sKey === "2" ? "М2" : sKey;
    if (!storeMap[label]) storeMap[label] = { inc: 0, exp: 0 };
    const a = Number(r.amount || 0);
    if (r.type === "Приход") { storeMap[label].inc += a; totalInc += a; }
    else                     { storeMap[label].exp += a; totalExp += a; }
  });

  const net = totalInc - totalExp;
  const f   = n => n.toFixed(2) + " €";
  const cls = n => n >= 0 ? "color:var(--green)" : "color:var(--red)";
  const multiStore = Object.keys(storeMap).length > 1;

  const storeRows = Object.entries(storeMap).map(([name, v]) => {
  ...
  el.innerHTML = `
    <div class="filter-summary-box">
      <div class="fs-header">📊 Резултат от филтъра — <strong>${data.length}</strong> записа</div>
      <div class="fs-totals">
        <div class="fs-total-item">
          <span class="fs-label">Приходи</span>
          <span class="fs-value" style="color:var(--green)">${f(totalInc)}</span>
        </div>
        <div class="fs-total-item">
          <span class="fs-label">Разходи</span>
          <span class="fs-value" style="color:var(--red)">${f(totalExp)}</span>
        </div>
        <div class="fs-total-item">
          <span class="fs-label">Салдо</span>
          <span class="fs-value" style="${cls(net)};font-size:1.1rem;font-weight:800">${f(net)}</span>
        </div>
      </div>
